...
class Ad extends DatabaseObject {
  ...
  protected function validate() {
    ...
    // elseif (!has_valid_url_format($this->link)) {
    //   $this->errors[] = "Link must be a valid format.";
    // }

    if (is_blank($this->imageLink)) {
      $this->errors[] = "Image cannot be blank";
    }
    // elseif (!has_valid_url_format($this->imageLink)) {
    //   $this->errors[] = "Image link must be a valid format.";
    // }
  ...
  static public function ad_query($station, $position) {
    $sql = "SELECT * FROM ads ";
    $sql .= "WHERE position=" . $position . " ";
    $sql .= "AND visible=1 ";
    $sql .= "ORDER BY position = 0, position, id DESC ";
    $sql .= "LIMIT 5 ";
    $results = Ad::find_by_sql($sql);
      ...
      <table class="list table">
        ...
        <?php foreach($results as $ad) {
          ...
          <tr>
            ...
            <td><?php $stations = (unserialize($ad->station));
            $stationArray = [];
            ...
            <td class="text-center align-middle">
              ...
              <input type="checkbox" name="ad[visible]" value="1"<?php if($ad->visible()) { echo " checked"; } ?> />
            </td>
